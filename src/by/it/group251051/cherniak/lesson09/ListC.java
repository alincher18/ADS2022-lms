package by.it.group251051.yourname.lesson09;
...
public class ListC<E> implements List<E> {
    ...
    public void add(int index, E element) {
        if (index < 0 || index > size) {
            throw new IndexOutOfBoundsException(getOutOfBoundsErrorMessage(index, size));
        }
    ...
    private String getOutOfBoundsErrorMessage(int index, int size) {
        return String.format("Index: %d is out of bounds (Size: %d)", index, size);
    }
    
    private void ensureCapacity(int minCapacity) {
        if (list.length < minCapacity) {
            int newCapacity = Math.max(list.length * 3 / 2 + 1, minCapacity);
